...
GeoIP* GeoIP_open (const char * filename, int flags) {
	...
	if (gi->GeoIPDatabase == NULL) {
		...
		// TWINDRILLS-CHANGE
#if defined(_WIN32)
		unsigned int position = ftell(gi->GeoIPDatabase);
		fseek(gi->GeoIPDatabase, 0, SEEK_END);
		buf.st_size = ftell(gi->GeoIPDatabase);
		fseek(gi->GeoIPDatabase, position, SEEK_SET);
