...
#include "Engine/Platform/Linux/Linux_EntryPoint.h"
...
void Linux_Platform::Sleep(float ms)
{
	usleep(ms * 1000);
}

int Linux_Platform::Get_Timestamp()
{
    time_t ltime;
    time(&ltime); 
...
double Linux_Platform::Get_Ticks()
{
	return platform_get_ticks();
}

void Linux_Platform::Crack_Path(const char* path, std::vector<std::string>& segments)
{
	StringHelper::Split(path, '/', segments);
}

std::string Linux_Platform::Join_Path(std::string a, std::string b)
{
...
std::string Linux_Platform::Normalize_Path(std::string value)
{
...
bool Linux_Platform::Is_Path_Absolute(std::string value)
{
	return !Is_Path_Relative(value);
}

bool Linux_Platform::Is_Path_Relative(std::string value)
{
	if (value.size() <= 1)
	{
		return true;
...
std::string Linux_Platform::Get_Absolute_Path(std::string value)
{	
...
std::string Linux_Platform::Extract_Directory(std::string a)
{
...
std::string Linux_Platform::Extract_Filename(std::string a)
{
	int index = a.find_last_of("/\\");
	if (index == std::string::npos)
	{
...
std::string Linux_Platform::Extract_Basename(std::string a)
{
	int index = a.find_last_of("/\\");
...
std::string Linux_Platform::Extract_Extension(std::string a)
{
	int index = a.find_last_of(".");
	if (index == std::string::npos)
	{
...
bool Linux_Platform::Is_File(const char* path)
{
	if (access(path, 0) != 0)
	{
		return false;
...
bool Linux_Platform::Is_Directory(const char* path)
{
	if (access(path, 0) != 0)
	{
		return false;
...
bool Linux_Platform::Create_Directory(const char* path, bool recursive)
{
	if (recursive == true)
	{
...
std::string	Linux_Platform::Get_Working_Dir()
{
	char buffer[512];
	getcwd(buffer, 512);
...
void Linux_Platform::Set_Working_Dir(const char* path)
{
	chdir(path);
}

bool Linux_Platform::Copy_File(const char* src, const char* dst)
{
	if (Is_File(dst) == true)
	{
		unlink(dst);
...
bool Linux_Platform::Delete_File(const char* from)
{
	return unlink(from) == 0;
}

int Linux_Platform::Get_Stack_Trace(StackFrame* frames, int max_frames, void* platform_specific, int offset)
{	
...
void Linux_Platform::Resolve_Stack_Frame_Info(StackFrame& frame, DecodedStackFrame& output)
{	
	int status = 0;
	size_t size = 0;
    Dl_info symbol_info;
	...
	{
		sprintf(output.Name, "0x%08x+0x%08x [0x%08x]", reinterpret_cast<int>(void_addr), (offset), reinterpret_cast<int>(void_addr));
	}
	else
	{
		sprintf(output.Name, "%s+0x%08x [0x%08x]", output.Name, (offset), reinterpret_cast<int>(void_addr));
	}
	...
	// Attempt to get DWARF filename and line number.
	//dwarf_addr2line(frame.Address, frame.File, StackFrame::MAX_STRING_SIZE, frame.Line);
	

	/*
	DwarfInfo dwarf_info;
	if (dwarf_addrinfo(frame.Address, &dwarf_info))
	{
	...
	*/
	/*
	char** messages = backtrace_symbols(reinterpret_cast<void**>(&frame.Address), 1);
	...
	char* open_parent = NULL;
	char* close_parent = NULL;
	char* open_bracket = NULL;
	char* close_bracket = NULL;

	for (int i = 0; i < len; i++)
	{
		if (message[i] == '(')	open_parent		= message + i;
		if (message[i] == ')')	close_parent	= message + i;		
		if (message[i] == '[')	open_bracket	= message + i;
		if (message[i] == ']')	close_bracket	= message + i;		
	}

	char* start_filename = message;
	char* end_filename   = (open_parent == NULL ? open_bracket - 1 : open_parent);
	char* start_name	 = (open_parent == NULL ? NULL : open_parent + 1);
	char* end_name		 = (close_parent == NULL ? NULL : close_parent);

	if (start_filename != NULL && end_filename != NULL)
	{
		strncpy(frame.File, start_filename, end_filename - start_filename);
	}

	if (start_name != NULL && end_name != NULL)
	{
		strncpy(frame.Name, start_name, end_name - start_name);

		int demangle_status = 0;
		int size = 0;
		char* demangled = abi::_cxa_demangle(frame.Name, NULL, &size, &status);

		if (demangled != NULL && demangle_status == 0)
		{			
			strncpy(frame.Name, demangled, size);
		}
	}

	printf("File:%s Name:%s\n", frame.File, frame.Name);

	frame.Line   = 0;		// No line numbers on linux, have to do some addr2line stuff offline to get this.
	frame.Column = 0;
	*/
}

bool Linux_Platform::List_Directory(const char* path, std::vector<std::string>& entries, ListDirectoryFlags::Type flags)
{
...
int Linux_Platform::Get_Core_Count()
{
	return (int)sysconf(_SC_NPROCESSORS_ONLN);
}

bool Linux_Platform::Spawn_Process(std::string process, std::string arguments)
{	
	pid_t pid = fork();
	int status;
	...
	case 0: 
		execl(process.c_str(), process.c_str(), arguments.c_str(), NULL); 
		exit(1);
...
std::string	Linux_Platform::Generate_UUID()
{
	uuid_t uuid;
