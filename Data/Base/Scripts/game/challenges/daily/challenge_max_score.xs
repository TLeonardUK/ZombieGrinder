...
public class Challenge_Max_Score : Challenge
{
    ...
    Challenge_Max_Score()
    {
        ID = "CHALLENGE_MAX_SCORE";
        Name = "#menu_game_setup_challenge_daily";
        Description = "#challenge_max_score_description";
    ...
    {
		int MaxRewards = Math.Min(4, 1 + ((<int>Max_Progress - 200000) / 100000));
    ...
    public event void Seeded_Setup()
    {
		...
		Max_Progress = <int>(Math.Rand(2, 6) * 100000);
        Description = Locale.Get("#challenge_max_score_description").Format(new object[] { <object>(<int>Max_Progress), <object>Possible_Maps[Map_Index] });
	}
