...
public class Statistic_Zombies_Frozen_Ignited : Statistic
{
	Statistic_Zombies_Frozen_Ignited()
	{
		ID 					= "ZOMBIES_FROZEN_IGNITED";
		Name 				= Locale.Get("#statistic_zombies_frozen_ignited_name");
		Description 		= Locale.Get("#statistic_zombies_frozen_ignited_description");
	...
	public event void On_Pawn_Status_Ailment(Event_Pawn_Status_Ailment evt)
	{
		...
		{
			if ((<Enemy>evt.Effected) != null && evt.Last_Ailment == StatusAilmentType.Frozen && evt.Ailment == StatusAilmentType.Burnt)
			{
				Value += 1.0f;
			}
		}
