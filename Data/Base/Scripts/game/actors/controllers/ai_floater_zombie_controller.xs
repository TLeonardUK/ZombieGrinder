...
[
	Name("AI Floater Zombie Controller"), 
	Description("Controls enemies that follow nearest enemy and shoot blood clots at them.") 
]
public class AI_Floater_Zombie_Controller : Pawn_Controller
{
	// -------------------------------------------------------------------------
	...
	private float	    m_walk_speed_acceleration;
	...
	const float ATTACK_INTERVAL				= 7.0f;
	const float ATTACK_DISTANCE				= 80.0f;
	
	const float ATTACK_DISTANCE_DISENGAGE	= 100.0f;

	AI_Floater_Zombie_Controller()
	{
	...
	state Idle
	{
		...
		event void On_Tick()
		{
			...
			{
				Pawn pawn = Possessed;
				Floater_Zombie zombie = <Floater_Zombie>pawn;
		...
		event void On_Tick()
		{
			if (Network.Is_Server())
			{
				...
				if (zombie.m_status_ailment != StatusAilmentType.Shocked)
				{
					...
					if (chase_distance <= ATTACK_DISTANCE && path_planner.Target_In_LOS)
					{
				...
				// Lerp to walk speed delta.
				m_walk_speed_acceleration = Math.Lerp(m_walk_speed_acceleration, 1.0f, 1.0f * Time.Get_Delta_Seconds());
				...
				Move_Towards(Position + chase_vector, Get_Walk_Speed() * m_walk_speed_acceleration, false, 0);
							
				// Check if anything destroyable is in our way.
				Check_For_Destroyables(chase_vector);

				// Fire at players in range.
				Fire_If_In_Range();

				// Turn towards direction of movement.
				if (chase_vector.Y > 0.1f)
				{
					pawn.Turn_Towards(Direction.S);
				}
				else if (chase_vector.Y < 0.1f)
				{
	...
	state Attacking
	{
		...
		event void On_Tick()
		{
			if (Network.Is_Server())
			{
				...
				{
					float chase_distance = (m_chase_target.Center - Center).Length();
					if (chase_distance > ATTACK_DISTANCE_DISENGAGE || !path_planner.Target_In_LOS)
					{
				...
				// Lerp to walk speed delta.
				m_walk_speed_acceleration = Math.Lerp(m_walk_speed_acceleration, 0.0f, 1.0f * Time.Get_Delta_Seconds());
				...
				// Face target
				Vec3 target_delta = (m_chase_target.Collision_Center - zombie.Collision_Center);
				if (target_delta.Y > 0.1f)
				{
					pawn.Turn_Towards(Direction.S);
				}
				else if (target_delta.Y < 0.1f)
				{
					pawn.Turn_Towards(Direction.N);
				}
	...
	void Fire_If_In_Range()
	{	
		Pawn pawn = Possessed;
		Floater_Zombie zombie = <Floater_Zombie>pawn;
		float delta = Time.Get_Delta_Seconds();

		if (m_chase_target != null && path_planner.Target_In_LOS)
		{
			// Fire the pawns weapon (pew pew pew).	
			m_attack_timer -= delta;

			if (m_attack_timer <= 0.0f && !pawn.IsParalyzed())
			{
				zombie.Fire_Projectile(Vec3(m_chase_target.World_Bounding_Box.Center(), 0.0f));
	...
	{		
		...
		{	
            ...
            if (m_attack_timer <= 0.0f && !pawn.IsParalyzed())
            {
                zombie.Fire_Projectile(Vec3(potential_targets[0].World_Bounding_Box.Center(), 0.0f));
                m_attack_timer = ATTACK_INTERVAL;
            }
		}
	}
