...
using game.weapons.enemies.weapon_fodder_zombie;
using game.difficulty.difficulty_manager;
 
[
	Placeable(false), 
	Name("Fodder Zombie"), 
	Description("Greyish colored zombie that just wanders around dumbly.") 
]
public class Fodder_Zombie : Enemy
{  
	// -------------------------------------------------------------------------
	...
	const float DECAPITATION_CHANCE		= 0.2f;
	const int	SCORE_INCREASE			= 5;
	const int	HIT_SCORE_INCREASE		= 1;
	const int	XP_INCREASE				= 50;

	const string WALK_ANIMATION			= "enemy_fodder_zombie_walk_";
	const string DECAP_WALK_ANIMATION	= "enemy_fodder_zombie_walk_nohead_";
	...
	Fodder_Zombie()
	{
		...
		if (Network.Is_Server())
		{
			...
			Give_Weapon(typeof(Weapon_Fodder_Zombie));
	...
	override void On_Death(Actor instigator, Item_Archetype weapon_type)
	{
		...
		{
			FX.Spawn_Untracked("fodder_zombie_die_explosion", Vec3(World_Bounding_Box.Center(), 0.0f), 0.0f, Layer);
		}
		else if (Is_Decapitated)
		{
			FX.Spawn_Untracked("fodder_zombie_die_decapitated",  this.Collision_Center, 0.0f, Layer);
		}
		else
		{
			FX.Spawn_Untracked("fodder_zombie_die",  this.Collision_Center, 0.0f, Layer);
		}
		...
		evt.Coin_Reward			= "coin_splash_tiny";
        ...
        Item_Archetype weapon_item)
	{
		...
		if (accepted == true)
		{
			...
			{
				// Spawn a hit effect.
				FX.Spawn_Untracked("fodder_zombie_hit", position.Offset(dir, HIT_OFFSET_FX_AMOUNT), dir, Layer);
	...
	void Update_Sprites()
	{
		...
		if (m_status_ailment == StatusAilmentType.Shocked)
		{
			walk_anim = "enemy_fodder_zombie_shocked_";
			anim_speed = 1.0f;
			ignore_oneshot = true;
		}
	...
	{	
		...
		{
			Effect_Name = "fodder_zombie_decap_spray";
			Visible		= true;
			Paused		= true;
			One_Shot	= true;
			Offset		= Vec3(16, 15, 0);
