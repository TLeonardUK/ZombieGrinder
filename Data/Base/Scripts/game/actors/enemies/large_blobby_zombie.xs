...
using game.weapons.enemies.weapon_large_blobby_zombie;
using game.difficulty.difficulty_manager;
 
[
	Placeable(false), 
	Name("Large Blobby Zombie"), 
	Description("Blah") 
]
public class Large_Blobby_Zombie : Enemy
{  
	// -------------------------------------------------------------------------
	// Non-replicated variables.
	// -------------------------------------------------------------------------
	const float START_HEALTH			= 90.0f;
	...
	string WALK_ANIMATION				= "enemy_large_blobby_walk";
	string HURT_ANIMATION				= "enemy_large_blobby_hurt";
	string SHOCK_ANIMATION				= "enemy_large_blobby_shock";
	...
	Large_Blobby_Zombie()
	{
		...
		if (Network.Is_Server())
		{
			...
			Give_Weapon(typeof(Weapon_Large_Blobby_Zombie));
	...
	override void On_Death(Actor instigator, Item_Archetype weapon_type)
	{
		...
		FX.Spawn_Untracked("blobby_zombie_die",  this.Collision_Center, 0.0f, Layer);
		...
		// Spawn 3 small zombie versions!
        if (Network.Is_Server())
        { 
		    Log.Write("Spawning small blobby enemies.");
		    Vec4 spawn_area = Vec4(this.Position.X, this.Position.Y, 0.0f, 0.0f) + this.Collision_Box;
		    for (int i = 0; i < 3; i++)
		    {			
			    Enemy_Manager.Spawn_Enemy(Enemy_Type.Small_Blobby, spawn_area, this.Layer, this.Team_Index);
		    }
        }
	}
    ...
    {
        return "item_hat_enemy_large_blobby";
    }
